---
description: Create a Product Requirements Document for this Next.js + Supabase + Fumadocs project
...
- Reference the existing vertical-slice layout (`src/features/*`, App Router route groups, `src/lib/supabase/*`)
- Key design patterns: Server Components by default, Server Actions for mutations, middleware for session refresh
- Data flow: Server Component → feature action → Supabase (SSR client)

**7. Tools/Features**
- Detailed feature specifications
- Note which routes live in which route group (`(auth)`, `(marketing)`, `(dashboard)`)
- Identify feature-slice boundaries under `src/features/`
- Call out any Server Actions, Route Handlers, or middleware behavior

**8. Technology Stack**
- This project is fixed to: Next.js 16, React 19, TypeScript, Supabase (SSR), shadcn/ui, Tailwind v4, Biome, Vitest
...
- Session handling via `@supabase/ssr` + `middleware.ts`
...
- Server Actions: file location, exported names, input Zod schemas, return shapes
- Route Handlers: path, method, auth requirements, request/response shapes
- Example payloads

**11. Success Criteria**
- MVP success definition
- Functional requirements (use ✅ checkboxes)
- Quality indicators (type check clean, Biome clean, tests green)
- User experience goals (Core Web Vitals, SEO where relevant)
