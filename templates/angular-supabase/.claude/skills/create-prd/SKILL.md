---
description: Create a Product Requirements Document for this Angular 21 + Supabase project
...
- Reference the existing vertical-slice layout (`src/app/features/*`, `src/app/shared/*`)
- Key design patterns: standalone components, signals-first reactivity, zoneless change detection, lazy-loaded routes
- Data flow: component → feature service → `SupabaseService` → Supabase

**7. Tools/Features**
- Detailed feature specifications
- Identify feature-slice boundaries under `src/app/features/`
- Call out any route guards, lazy routes, or cross-cutting services
- Note signal inputs/outputs (`input()`, `output()`) and reactive resources (`resource()`, `httpResource`)

**8. Technology Stack**
- This project is fixed to: Angular 21 (standalone, zoneless), TypeScript 5.8 strict, Supabase, Tailwind CSS v4, Biome (TS/JS only), Vitest + @testing-library/angular
...
- Environment config in `src/environments/environment.ts` and `environment.development.ts` (no `.env` files at runtime — build-time file replacement via `angular.json`)
- Auth guards in `src/app/shared/guards/`
- Row Level Security (RLS) policies — in scope per feature
- Security scope (in-scope and out-of-scope)

**10. API Specification** (if applicable)
- Feature service methods: signatures, return types (signals vs observables vs promises)
...
- User experience goals (bundle size, Core Web Vitals, accessibility)
...
- Include Angular/Supabase-specific risks (signal/effect lifecycle, RLS misconfiguration, environment file leakage) where relevant
