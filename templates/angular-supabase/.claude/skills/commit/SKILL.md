---
description: Create an atomic commit for current changes in this Angular 21 + Supabase project
---
...
- `dist/` build output, `node_modules/`, `.angular/cache/`
...
```bash
pnpm lint             # Biome check (TS/JS — does not lint Angular templates)
pnpm tsc --noEmit     # TypeScript strict check (Angular compiler enforces template checks)
...
fix(home): resolve signal read outside effect
refactor(shared): convert SupabaseService to inject() pattern
chore(deps): bump @angular/core to 21.0.x
```
...
- Noted gotcha: auth guard must read session signal, not call supabase.auth directly
