...
Each slice runs in its own session: `/piv-loop:prime`, read this roadmap, `/piv-loop:generate-plan <the slice's
generate line>`, review and confirm the plan, `/piv-loop:execute-plan <plan>`, `/piv-loop:review-plan-code <plan> <diff>`,
...
### A — <name>
- **Generate with:** `/piv-loop:generate-plan <terse statement of intent>`
...
- **Generate with:** `/piv-loop:generate-plan pure function mapping <input> → <output>, no I/O`
