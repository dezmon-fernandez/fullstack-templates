---
description: "Create comprehensive feature plan with deep codebase analysis and research — directive: the plan specifies the contract and a pseudocode implementation plan, never literal code"
---
...
- `path/to/model.py` (lines 100-120) - Why: Data model structure to follow
...
## CONTRACT (what must become observably true)

<The interface the implementation must satisfy. Define shapes and behavior — not the body.>

### Public Surfaces

<Signatures/types only — name + inputs + outputs. e.g. `function/method/endpoint/schema`. No body.>

### Data Models & Specs

<Defining types and data models is encouraged — they are the contract. Field name + type + constraint + meaning; nullability and what null means. Declare the shape; not its validator/mapping logic.>

### Behavioral Rules

<Observable rules a test could check — pre/post-conditions, invariants, error behavior.>
...
<High-level plan as a sketch — ordered steps and pseudocode. Reference the patterns/files to mirror. The implementation agent translates this into real code. NO literal, paste-ready code here.>
...
- Add edge case tests for every public-method path
...
IMPORTANT: Execute every task in order, top to bottom. Each task is atomic and independently testable. Each task states WHAT must become true and sketches the HOW in pseudocode — never literal, paste-ready code.
...
- **MIRROR**: Follow a pattern from elsewhere in codebase

### {ACTION} {target_file}

- **CONTRACT**: {What must become observably true — signature/type/shape/behavior the task delivers}
- **APPROACH**: {The HOW as pseudocode — the algorithm's shape, no literal code}
- **PATTERN**: {Reference to existing pattern - file:line to mirror}
- **IMPORTS**: {Libraries/dependencies it will need}
- **EDGE CASES**: {The paths this task must handle + test}
...
<List the specific edge cases that must be tested — covering every path of each public method.>
...
- [ ] Every path of each public method has an edge-case test
...
- [ ] Every public-method path enumerated in edge cases
...
- [ ] The contract defines surfaces, data models, and behavior; the implementation plan is pseudocode — no literal code
...
**No Literal Code**: The plan specifies the contract and a pseudocode implementation plan; the execution agent writes the code
